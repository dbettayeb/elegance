...
export default async function TemplateEmbed({ params, searchParams }: Props) {
  ...
  const mockWedding: Wedding = {
    ...
    ar_font_theme: (['classic', 'modern', 'calligraphique', 'contemporain'] as const).includes(ar_font_theme as never)
      ? (ar_font_theme as 'classic' | 'modern' | 'calligraphique' | 'contemporain')
      : 'classic',
    status: 'active',
